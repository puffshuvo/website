...
function changeQuantity(index, change) {
  if (index >= 0 && index < cartItems.length) {
    ...
    updateCartDisplay();
    saveCartState();
    // Recalculate total and re-render receipt so HTML updates immediately
...
async function downloadPDF() {
  ...
  if (totalElement) {
    ...
    firstToken = parseFloat(totalText.split(" ")[0]) || 0;
  }
...
async function generatePDFDocument() {
  // Form and cart validation (from downloadPDF)
  const name = document.getElementById("input-name")?.value.trim() || "";
  const phone = document.getElementById("input-phone")?.value.trim() || "";
  const address = document.getElementById("input-address")?.value.trim() || "";
  const paymentSelected = document.querySelector('input[name="payment"]:checked');
  ...
  // Hide quantity buttons and clear cart button
  qtyButtons.forEach(btn => btn.style.display = "none");
  items.forEach(item => item.classList.add("pdf-quantity-only"));
  ...
  try {
    ...
    qtyButtons.forEach(btn => btn.style.display = "flex");
    items.forEach(item => item.classList.remove("pdf-quantity-only"));
